...
def create_omnichannel_variables(df):
...
def analyze_channel_data(df):
    # Alışveriş kanallarındaki müşteri sayısının, toplam alınan ürün sayısı ve toplam harcamaların dağılımına bakınız.
    return df.groupby('order_channel').agg({
        "master_id": "count",
        "total_order_num_omnichannel": "sum",
        "total_customer_value_omnichannel": "sum"
    })

def top_customers_by_value(df, n=10):
    # En fazla kazancı getiren ilk n müşteriyi sıralayınız.
    return df.groupby("master_id").agg({
        "total_customer_value_omnichannel": "sum"
    }).sort_values("total_customer_value_omnichannel", ascending=False).head(n)

def top_customers_by_orders(df, n=10):
    # En fazla siparişi veren ilk n müşteriyi sıralayınız.
    return df.groupby("master_id").agg({
        "total_order_num_omnichannel": "sum"
    }).sort_values("total_order_num_omnichannel", ascending=False).head(n)
...
female_shoppers = loyal_customers[loyal_customers['interested_in_categories_12'].str.contains('KADIN', case=False, na=False)]

target_customer_ids = female_shoppers['master_id']
target_customer_ids.to_csv('yeni_marka_hedef_müşteri_id.csv')
